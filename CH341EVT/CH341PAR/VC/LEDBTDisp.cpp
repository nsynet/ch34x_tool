...
{ 
	...
	if(CPublic::m_open)
	{
		try
		{
//			CH341SetTimeout(CPublic::mIndex, 1000, 1000);
			if (m_add0==0)  //用地址0方式发送数据
			{
				if(!CH341MemWriteAddr0( CPublic::mIndex, mBuf, &mLen ) )
				{
//					CH341SetTimeout(CPublic::mIndex, 0xFFFFFFFF, 0xFFFFFFFF);
			...
			{
				if(!CH341MemWriteAddr1( CPublic::mIndex, mBuf, &mLen ) )
				{
//					CH341SetTimeout(CPublic::mIndex, 0xFFFFFFFF, 0xFFFFFFFF);
...
{
  	if(CPublic::m_open)  //设备打开,在当前页被激活时刷新LED,按钮状态
	{
		CLEDBTDisp::OnCheck8(); 
...
void CLEDBTDisp::enablebtn(BOOL bEnable)
{
	::EnableWindow(::GetDlgItem(this->GetSafeHwnd(), IDC_BtRefresh), bEnable);
	
	if(this->formshow==TRUE && bEnable)  //数字I/O窗口已初始化,刷新LED,按拔码开关状态
	{
		this->OnCheck8(); 
		this->OnBtRefresh();
	}
