...
void COtherPage::OnButtonI2cread()		//读I2C
{
	...
	{
		MessageBox("请输入数据单元地址","CH341",MB_OK|MB_ICONSTOP);
		return;
	}
	else if(strlen(m_devaddr) == 0)
	{
		MessageBox("请输入设备地址","CH341",MB_OK|MB_ICONSTOP);
		return;
	}
	if(strlen(m_dataaddr) > 2 || strlen(m_devaddr) > 2)
	{
		MessageBox("设备地址和数据单元地址都应不大于十六进制FFH","CH341",MB_OK|MB_ICONSTOP);
		return;
	}
...
void COtherPage::OnButtonI2cwrite()			//写I2C
{
	...
	{
		MessageBox("请输入设备地址、数据单元地址和数据","CH341",MB_OK|MB_ICONSTOP);
		return;
	}
	if(strlen(m_dataaddr) > 2 || strlen(m_devaddr) > 2 || strlen(m_data) > 2)
	{
		MessageBox("设备地址、数据单元地址和待写入数据都应不大于十六进制FFH","CH341",MB_OK|MB_ICONSTOP);
		return;
	}
	...
	if(CPublic::m_open)
	{
		...
		{
			MessageBox("I2C写数据成功！","CH341",MB_OK|MB_ICONSTOP);			
		} 
	}
	else
	{
